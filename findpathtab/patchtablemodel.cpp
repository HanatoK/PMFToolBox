...
QVariant PatchTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
  if (role == Qt::DisplayRole && orientation == Qt::Horizontal) {
    if (mDimension > 0) {
      // horizontal header
      if (section == 0) return QString("Center");
      else if (section == int(mDimension + 1)) return QString("Potential");
      else {
...
QVariant PatchTableModel::data(const QModelIndex &index, int role) const
{
  ...
  if (role == Qt::DisplayRole) {
    ...
    if (mDimension == 0 || mPatchList.size() == 0) return QVariant();
    if (row_index >= 0 && column_index >= 0) {
      ...
      else if (column_index == int(mDimension + 1)) return mPatchList[row_index].mValue;
      else {
...
bool PatchTableModel::insertRows(int position, int rows, const QModelIndex &index)
{
  Q_UNUSED(index);
...
bool PatchTableModel::removeRows(int position, int rows, const QModelIndex &index)
{
  Q_UNUSED(index);
...
bool PatchTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
  if (index.isValid() && role == Qt::EditRole) {
    ...
    if (col == 0) {
      std::vector<double> fields = splitStringToNumbers<double>(value.toString());
      if (fields.size() != mDimension) return false;
...
size_t PatchTableModel::dimension() const
{
  return mDimension;
}
...
QList<GridDataPatch> PatchTableModel::patchList() const
{
  return mPatchList;
}
