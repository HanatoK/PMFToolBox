#ifndef TURBOCOLORMAP_H
#define TURBOCOLORMAP_H

#include <qwt_color_map.h>

class TurboColorMap : public QwtLinearColorMap {
 public:
  TurboColorMap()
      : QwtLinearColorMap(QColor(48, 18, 59), QColor(122, 4, 3),
                          QwtColorMap::RGB) {
    for (int i = 1; i < 256 - 1; ++i) {
      addColorStop(double(i) / 256.0,
                   QColor(turbo_srgb_bytes[i][0], turbo_srgb_bytes[i][1],
                          turbo_srgb_bytes[i][2]));
    }
  }

 private:
  unsigned int turbo_srgb_bytes[256][3] = {
