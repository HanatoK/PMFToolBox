...
NAMDLogTab::NAMDLogTab(QWidget *parent)
    : QWidget(parent), ui(new Ui::NAMDLogTab),
  ...
  ui->tableViewAxis->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Stretch);
  connect(ui->pushButtonOpenNAMDLog, &QPushButton::clicked, this,
          &NAMDLogTab::loadNAMDLog);
  connect(ui->pushButtonOpenColvarsTrajectory, &QPushButton::clicked, this,
          &NAMDLogTab::openTrajectory);
  connect(ui->pushButtonRun, &QPushButton::clicked, this,
          &NAMDLogTab::runBinning);
  ...
  connect(ui->pushButtonRemoveAxis, &QPushButton::clicked, this,
          &NAMDLogTab::removeAxis);
  connect(&mLogReaderThread, &NAMDLogReaderThread::done, this,
          &NAMDLogTab::loadNAMDLogDone);
  connect(&mLogReaderThread, &NAMDLogReaderThread::progress, this,
          &NAMDLogTab::logReadingProgress);
  connect(&mBinningThread, &BinNAMDLogThread::done, this,
          &NAMDLogTab::binningDone);
  connect(&mBinningThread, &BinNAMDLogThread::progress, this,
          &NAMDLogTab::binningProgress);
}

NAMDLogTab::~NAMDLogTab() { delete ui; }
...
void NAMDLogTab::loadNAMDLogDone(NAMDLog log) {
  ...
  // debug information
  qDebug() << Q_FUNC_INFO
           << ": available energy terms: " << mLog.getEnergyTitle();
  qDebug() << Q_FUNC_INFO
           << ": available force terms: " << mLog.getForceTitle();
...
void NAMDLogTab::runBinning() {
  ...
  if (availableEnergyTitle.size() <= 0 && availableForceTitle.size() <= 0) {
    qDebug() << Q_FUNC_INFO
             << ": no available energy or force terms in the log file.";
    return;
  }
  selectEnergyTermDialog dialog(availableEnergyTitle, availableForceTitle,
                                this);
  if (dialog.exec()) {
    mSelectedEnergyTitle = dialog.selectedEnergyTitle();
    mSelectedForceTitle = dialog.selectedForceTitle();
    qDebug() << Q_FUNC_INFO
             << ": seleted energy terms:" << mSelectedEnergyTitle;
  ...
  mBinningThread.invokeThread(mLog, mSelectedEnergyTitle, mSelectedForceTitle,
                              trajectoryFileName, axes, columns);
}
...
void NAMDLogTab::binningDone(std::vector<HistogramScalar<double>> energyData,
  ...
  for (int i = 0; i < mSelectedEnergyTitle.size(); ++i) {
    const QString outputFileName =
        outputFilePrefix + "_" + mSelectedEnergyTitle[i].toLower() + ".dat";
    mEnergyHistogram[i].writeToFile(outputFileName);
  }
  for (int i = 0; i < mSelectedForceTitle.size(); ++i) {
    const QString outputFileName =
        outputFilePrefix + "_" + mSelectedForceTitle[i].toLower() + ".dat";
...
selectEnergyTermDialog::selectEnergyTermDialog(const QStringList &energyTitle,
                                               const QStringList &forceTitle,
                                               QWidget *parent)
    : QDialog(parent), mAvailableEnergyTitle(energyTitle),
      mAvailableForceTitle(forceTitle) {
  qDebug() << Q_FUNC_INFO;
  ...
  gLayout->addLayout(hLayout, gLayout->rowCount(), 0, 1,
                     gLayout->columnCount());
  setLayout(gLayout);
...
NAMDLogCLI::NAMDLogCLI(QObject *parent) : CLIObject(parent) {
  ...
  connect(&mLogReaderThread, &NAMDLogReaderThread::done, this,
          &NAMDLogCLI::loadNAMDLogDone);
  connect(&mBinningThread, &BinNAMDLogThread::progress, this,
          &NAMDLogCLI::binningProgress);
  connect(&mBinningThread, &BinNAMDLogThread::done, this,
          &NAMDLogCLI::binningDone);
}
...
void NAMDLogCLI::binningDone(std::vector<HistogramScalar<double>> energyData,
  ...
  for (int i = 0; i < mSelectedEnergyTitle.size(); ++i) {
    const QString outputFileName =
        mOutputPrefix + "_" + mSelectedEnergyTitle[i].toLower() + ".dat";
    mEnergyHistogram[i].writeToFile(outputFileName);
  }
  for (int i = 0; i < mSelectedForceTitle.size(); ++i) {
    const QString outputFileName =
        mOutputPrefix + "_" + mSelectedForceTitle[i].toLower() + ".dat";
    mForceHistogram[i].writeToFile(outputFileName);
  }
  emit allDone();
}
